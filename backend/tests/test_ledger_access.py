...
def stored(owner, **changes):
    return {
        **payload(),
        ...
        "owner_id": owner.id,
        "shared_group_id": (
            owner.shared_group_id if changes.get("account_type") == "shared" else None
        ),
        **changes,
    }
...
def ledger():
    ...
    with TestClient(app) as client:
        yield SimpleNamespace(
            db=db, client=client, owner=owner, partner=partner, outsider=outsider
        )
...
def pair(ledger, owner=None):
    ...
    run(
        ledger.db.accounts.insert_many(
            [
                {
                    "_id": personal_account,
                    "owner_id": owner.id,
                    "is_active": True,
                    "account_type": "personal",
                    "currency": "CAD",
                ...
                },
                {
                    ...
                    "currency": "CAD",
                    "is_liability": False,
                    "shared_group_id": owner.shared_group_id,
                },
            ]
        )
    )
    expense = stored(
        ...
        sub_category=TRANSFER_SUB_SHARED_FUNDING,
        account_id=str(personal_account),
        counter_account_id=str(shared_account),
    )
    income = stored(
        owner,
        category=TRANSFER_CATEGORY,
        ...
        account_type="shared",
        type="income",
        account_id=str(shared_account),
        counter_account_id=str(personal_account),
        linked_transaction_id=str(expense["_id"]),
    )
...
def test_clients_cannot_supply_link_ids(ledger, method):
    ...
    response = getattr(ledger.client, method)(
        url, json=payload(linked_transaction_id=str(target["_id"]))
    )
...
def test_private_transaction_cannot_be_mutated(ledger, method, target_owner):
    ...
    response = getattr(ledger.client, method)(
        f"/api/transactions/{target['_id']}", **kwargs
    )
...
def test_legacy_foreign_link_is_blocked_before_any_write(ledger, method):
    ...
    response = getattr(ledger.client, method)(
        f"/api/transactions/{own['_id']}", **kwargs
    )
    assert response.status_code == 404
    assert snapshot(ledger) == before
...
def test_invalid_existing_links_fail_without_partial_writes(ledger, method, broken):
    ...
    if patch:
        ...
        run(
            ledger.db.transactions.update_one(
                {"_id": expense["_id"]}, {"$set": {"linked_transaction_id": link}}
            )
        )
    before = snapshot(ledger)
    kwargs = {"json": payload()} if method == "put" else {}
    response = getattr(ledger.client, method)(
        f"/api/transactions/{expense['_id']}", **kwargs
    )
...
def test_partner_cannot_mutate_personal_entry_through_shared_twin(ledger, method):
    ...
    response = getattr(ledger.client, method)(
        f"/api/transactions/{income['_id']}", **kwargs
    )
    assert response.status_code == 404
    assert snapshot(ledger) == before
...
def test_owner_can_update_valid_funding_pair_from_either_side(ledger, side):
    ...
    assert {doc["linked_transaction_id"] for doc in saved} == {
        str(doc["_id"]) for doc in saved
    }
...
def test_create_shared_funding_generates_server_links(ledger):
    ...
    saved = snapshot(ledger)
    assert len(saved) == 2
    shared = next(doc for doc in saved if doc["account_type"] == "shared")
    assert shared["shared_group_id"] == ledger.owner.shared_group_id
    assert {doc["linked_transaction_id"] for doc in saved} == {
        str(doc["_id"]) for doc in saved
    }
...
def test_partner_can_edit_shared_entry_but_cannot_move_it_to_personal(ledger):
    ...
    insert(ledger, doc)
    response = ledger.client.put(
        f"/api/transactions/{doc['_id']}",
        json=payload(account_type="shared", amount=80),
    )
    assert response.status_code == 200, response.text
...
def test_suggestions_only_include_the_requested_ledger(ledger, scope, endpoint):
    ...
    insert(ledger, *docs)
    response = ledger.client.get(
        f"/api/transactions/{endpoint}",
        params={"account_type": scope, "category": "투자/저축"},
    )
    assert response.status_code == 200, response.text
    expected = (
        {"owner-personal"}
        if scope == "personal"
        else {"owner-shared", "partner-shared"}
    )
...
def test_merchant_lookup_ignores_private_and_foreign_categories(ledger, scope):
    ...
    response = ledger.client.get(
        "/api/transactions/merchants/lookup",
        params={"account_type": scope, "name": "same shop"},
    )
    assert response.json() == {
        "found": True,
        "category": wanted["category"],
        "sub_category": wanted["sub_category"],
    }
...
def test_private_only_merchant_is_not_discoverable_in_shared_lookup(ledger):
    ...
    response = ledger.client.get(
        "/api/transactions/merchants/lookup",
        params={"account_type": "shared", "name": "Private clinic"},
    )
...
def test_unlinked_user_has_no_shared_history_suggestions(ledger):
    ...
    response = ledger.client.get(
        "/api/transactions/merchants/all", params={"account_type": "shared"}
    )
    assert response.json() == []
...
def test_custom_category_named_like_funding_does_not_create_links(ledger):
    run(
        ledger.db.user_settings.insert_one(
            {
                "owner_id": ledger.owner.id,
                "custom_categories": {
                    "expense": {"Custom": [TRANSFER_SUB_SHARED_FUNDING]}
                },
            }
        )
    )
